...
impl Future for TimerFuture {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let mut shared_state = self.shared_state.lock().expect("lock should not fail");
...
impl TimerFuture {
    fn new(duration: Duration) -> Self {
        ...
        thread::spawn(move || {
            ...
            let mut shared_state = thread_shared_state.lock().expect("lock should not fail");
...
impl Executor {
    fn run(&self) {
        while let Ok(task) = self.ready_queue.recv() {
            let mut future_slot = task.future.lock().expect("lock should not fail");
